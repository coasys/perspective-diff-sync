...
use crate::{
    Perspective, PerspectiveDiff, LocalHashReference, HashAnchor, HashReference, AgentReference, ACTIVE_AGENT_DURATION, ENABLE_SIGNALS
};
...
pub fn commit(diff: PerspectiveDiff) -> SocialContextResult<HoloHash<holo_hash::hash_type::Header>> {
    ...
    let diff_entry_create = create_entry(diff_entry.clone())?;
    ...
    if *ENABLE_SIGNALS {
        ...
        remote_signal(diff_entry.diff.get_sb()?, recent_agents)?;
    };
